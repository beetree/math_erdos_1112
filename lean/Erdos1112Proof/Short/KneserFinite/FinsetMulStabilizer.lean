...
Ported from Mathlib's `Mathlib.Combinatorics.Additive.Kneser.FinsetMulStabilizer`
...
syntax used throughout this repository; no mathematical content was changed.
-/
...
lemma smul_finset_mulStab_subset (ha : a ∈ s) : a • s.mulStab ⊆ s :=
  (smul_finset_subset_smul ha).trans (Finset.subset_of_eq s.mul_mulStab)
...
lemma disjoint_smul_finset_mulStab_mul_mulStab :
  ...
  simp_rw [@not_imp_comm ((_ : Finset α) ⊆ _), ← smul_eq_mul, ← biUnion_smul_finset,
    disjoint_biUnion_right, Classical.not_forall]
  rintro ⟨b, hb, h⟩
