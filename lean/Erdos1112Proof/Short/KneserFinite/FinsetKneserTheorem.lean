...
Ported from Mathlib's `Mathlib.Combinatorics.Additive.Kneser.Multiplicative`
(source snapshot supplied out-of-band; that file is absent from the Mathlib
revision `a3a10db0e9d66acbebf76c5e6a135066525ac900` pinned by this project).
Adapted from the upstream module/`public import` syntax to the plain `import`
syntax used throughout this repository, and re-pointed at the local port of
`FinsetMulStabilizer`. A handful of lemma names used upstream (a newer
Mathlib snapshot) do not exist verbatim in the pinned Mathlib; those spots are
noted inline and either use the pinned equivalent or a short local lemma. No
mathematical content was otherwise changed.
-/
import Mathlib.Algebra.Ring.Divisibility.Basic
import Mathlib.Tactic.Abel
import Mathlib.Tactic.Linarith
import Erdos1112Proof.Short.KneserFinite.FinsetMulStabilizer
...
lemma mulStab_union (hs₁ : (s ∩ a • C.mulStab).Nonempty) (ht₁ : (t ∩ b • C.mulStab).Nonempty)
  ...
  have hxysub : (x * y) • C.mulStab ⊆ s ∩ a • C.mulStab * (t ∩ b • C.mulStab) :=
    hxyC.left_le_of_le_sup_left
      (hxyCsubC.trans <| subset_union_left.trans (Finset.subset_of_eq hx))
...
/-- Local replacement for `Nat.le_of_lt_add_of_dvd`, which is not present in the pinned Mathlib
snapshot: if `d` divides both `a` and `b`, and `a < b + d`, then `a ≤ b`. -/
private lemma le_of_lt_add_of_dvd_aux {a b d : ℕ} (h : a < b + d) (hda : d ∣ a) (hdb : d ∣ b) :
    a ≤ b := by
  obtain ⟨x, rfl⟩ := hda
  obtain ⟨y, rfl⟩ := hdb
  ...
  · simp [hd]
  · have hxy : x < y + 1 :=
      Nat.lt_of_mul_lt_mul_left (a := d) (by rw [mul_add, mul_one]; simpa [add_comm] using h)
    exact Nat.mul_le_mul_left d (Nat.lt_succ_iff.mp hxy)
...
@[to_additive]
private lemma card_mul_add_card_lt_aux (hC : C.Nonempty) (hs : s' ⊆ s) (ht : t' ⊆ t)
...
theorem mul_kneser :
  ...
  have hst₁ : #(s₁ * t₁) + #s₁ < #(s * t) + #s :=
    card_mul_add_card_lt_aux hC hs₁s ht₁t hCst hCst₁
  have hst₂ : #(s₂ * t₂) + #s₂ < #(s * t) + #s :=
    card_mul_add_card_lt_aux hC hs₂s ht₂t hCst hCst₂
