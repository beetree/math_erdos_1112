...
This paper is also an experiment in form. It is written to be \emph{read} rather
than merely checked: everything the proof needs is built from the ground up, the
governing intuition is stated before each construction and separated from the
estimates that pay for it, and the argument is illustrated throughout. Nothing is
assumed beyond comfort with elementary algebra and some experience reading proofs.
\S\ref{sec:onform} explains the choice and what it costs.
\end{abstract}
...
It is written unconventionally, and deliberately so. The conventions of research
mathematics reward compression: a proof is presented in the smallest number of
symbols that a specialist can expand, the intuition that generated it is discarded
once it has done its work, and the reader is trusted to reconstruct what was left
out. That is an efficient way to communicate with fifty people and an effective
way to exclude everyone else. It also, in our experience, hides errors --- the
compressed step is exactly where the reader stops checking.

So this paper tries the opposite. Every prerequisite is built in, from remainder
arithmetic to the completeness of the real line. The governing idea of each
construction is stated in plain language and then \emph{separated} from the
estimates that make it work, so that the reader can see which lines carry the
mathematics and which merely pay for constants. The argument is illustrated
throughout. Where a routine expansion would bury the idea it serves, the expansion
goes to an appendix rather than into the reader's way. The result is roughly three
times the length of a conventional write-up of the same theorems.

We think that is a good trade, and we would like to see the experiment repeated by
others; but the reader should know what it costs. It costs length. It costs the
particular pleasure of a dense argument admired by people who can already read it.
And it forgoes the presumption --- comfortable but rarely examined --- that
difficulty of exposition is evidence of depth of content. What it does not cost is
rigour: nothing here is asserted without proof, the finite layer is
machine-checked three independent ways, and the entire dichotomy is verified in
Lean~4 (\S\ref{sec:computer}). A conventional presentation of the same results is
available in the repository for readers who prefer one.
...
\item \textup{(Existence.)} If $d_2 \ge k+1$, you have a winning strategy:
      ...
      (Any larger announcement works too, for a reason worth pausing on: raising
      $r$ only \emph{shrinks} the set of sequences the Adversary may legally play,
      so a strategy that beats them all at $r = 192 d_2$ beats the survivors at
      any $r \ge 192 d_2$. This is Exercise~\ref{chk:monotone-r}.)
...
\]
and $m_1 + \cdots + m_k$ ranges over every integer $\ge 0$
(Exercise~\ref{chk:s-range} showed this ranging argument for $\ge k$; here the
$i_t$ may be $0$, so even every integer $\ge 0$). By Lemma~\ref{lem:kA-prefix}, $kA \supseteq
...
\begin{bigidea}
The logical shape of Case T is the pattern of every computer-assisted step in
this work: the \emph{infinite} part is a slope comparison a student can check
(budget grows at rate $\le 2/3$, target at rate $1$), and the \emph{finite} part
is large but mindless. The machine never supplies an idea --- only stamina.
\S\ref{sec:computer} describes the checking apparatus in full.
\end{bigidea}
