% Short reconstruction: SHARP is proved here without certificate tables.
...
We address the formulation recorded as Erd\H{o}s Problem~1112~\cite{E1112},
inspired by a remark of Erd\H{o}s and Graham~\cite{ErGr80}.
The earlier proof~\cite{Land26} established the dichotomy with the explicit
bound $192d_2$ and was formally verified in Lean. The present reconstruction
improves the positive bound and replaces the finite certificate layer with
an elementary proof. The verification scope is recorded in Appendix~\ref{sec:formal}.
...
sharp finite input is Theorem~\ref{thm:sharp}, proved in the appendix.
...
\end{equation}
\subsection{The density shortcut and the finite input}
For $X\subseteq\NN_0$ write
$\dd(X)=\liminf_{N\to\infty}|X\cap[0,N]|/(N+1)$.
\begin{lemma}[Kneser shortcut]\label{lem:density}
...
$\dd(P)\ge1/c>1/k$. The lower-density form of Kneser's theorem
\cite{Kneser53,ENS88} states that either $\dd(kP)\ge k\dd(P)$ or $kP$
is eventually periodic. The first alternative is impossible because density
is at most one. Since $0\in P$, the set $kP$ contains the infinite set $P$;
its eventual periodicity therefore supplies a congruence-class tail.
\end{proof}
For a finite multiset $S$, write
$\ssum(S)=\{\sum_{x\in T}x:T\text{ is a submultiset of }S\}$.
\begin{theorem}[Finite interval lemma (SHARP)]\label{thm:sharp}
...
\end{theorem}
The elementary proof in Appendix~\ref{sec:sharp-proof} uses no search or
certificate list.
...
\end{lemma}
\begin{proof}
By SHARP choose $S=\{\delta_1,\ldots,\delta_m\}$, $m\le M-1\le k-1$,
...
Park $k-1-m$ summands at $p_0=0$. The last summand is a dial $p_n$.
...
Thus assume $M=k$. If $|G|\ge3$, use Lemma~\ref{lem:slots}. The remaining
alphabet is $G=\{\delta,k\}$ with $1\le\delta<k$ and $\gcd(\delta,k)=1$.
...
eventually; then $p_n\le(k-e\gamma)n$, and the density shortcut applies.
...
For $k=2u$, $u\ge2$, choose $\sigma_0<\sigma_1$ with
...
\section{The universal lacunary certificate}
...
\subsection{Reduction to a dense triple}
...
\end{proof}
\begin{proposition}[Inductive funnel]\label{prop:funnel}
Assuming SHARP below maximum $M$, it suffices at $M$ to treat triples
...
\begin{equation}\label{eq:crude-budget}
 (a-K-1)M\ge a(K+1)-1.
\end{equation}
For $a=3$ all unit residues are already excluded.
...
The following parts of the new argument are checked in Lean. File paths are
relative to \texttt{lean/Erdos1112Proof/}.
\begin{center}\small
\begin{tabular}{ll}
\toprule
...
Inductive funnel & \texttt{sharpAt\_of\_funnel}\\
\bottomrule
\end{tabular}
\end{center}
...
The migration of the non-existence and finite interval proofs to the argument
of this paper is in progress. The previously verified proofs temporarily
remain in the development; their verification does not certify the new
Kneser shortcut or the symmetric residue argument.
The definitions in \texttt{lean/Erdos1112.lean} allow repeated summands,
phrase gap bounds additively, and include a proved equivalence between
natural and integer ratio formulations. The repository records the current
migration status and reproduction commands.
